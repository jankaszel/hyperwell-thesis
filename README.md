...
You can use the [`wc` utility](https://www.freebsd.org/cgi/man.cgi?wc) to perform a word count of the thesis' contents. Simply run `wc -w chapters/*.md` (or `make stats`) to gather an approximate. Running it on the Markdown sources should omit words generated by pandoc/LaTeX and give a better approximation.
